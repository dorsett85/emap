...
from api.Classes.QueryHelper import QueryHelper
...
qh = QueryHelper('SELECT file FROM api_migration').fetchall_array()
...
for file in files:
    if qh.results:
        if os.path.basename(file) not in [row['file'] for row in qh.results]:
            add_migration(file)
            new_migrations = True
    else:
        add_migration(file)
